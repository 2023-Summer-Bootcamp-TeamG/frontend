...
export default function EditPage() {
  ...
  const getPhotoDetail = async () => {
    try {
      ...
      console.log('in the getPhoto : ', stickers, textboxes, drawings);
  ...
  const updatePhoto = async () => {
    ...
    try {
      const response = await apiV1Instance.put(`/photos/${id}/`, data);
      console.log(response.data);
      navigate('/album');
    } catch (error) {
      alert(error);
  ...
  console.log('outside, store : ', drawings, stickers, texts);
